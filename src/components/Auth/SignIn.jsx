...
const SignIn = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch("https://backend-health-connect.vercel.app/auth/login", {
                method: "POST",
                headers: {
                ...
                <div className="w-full max-w-lg text-center bg-white p-8 rounded-lg shadow-xl">
                    ...
                    <form className="space-y-4 shadow-lg p-10" onSubmit={handleSubmit}>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-3  disabled:opacity-50 px-6 py-2 rounded-xl font-semibold text-white shadow-md transition-all duration-300 bg-[linear-gradient(90deg,_rgba(36,0,9,1)_0%,_rgba(121,9,13,1)_77%,_rgba(214,69,69,1)_94%)] hover:shadow-[0_0_20px_4px_rgba(214,69,69,0.5)]"
                        >
